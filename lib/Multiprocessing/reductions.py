...
import xcelerate
import xcelerate.utils.hooks
from xcelerate._namedtensor_internals import check_serializing_named_tensor
...
class StorageWeakRef:
    ...
    def __init__(self, storage):
        ...
        # Save a direct reference to _free_weak_ref because the `xcelerate` module
        # might be cleared during Python shutdown before this module is cleared.
        self._free_weak_ref = xcelerate.Storage._free_weak_ref  # type: ignore[attr-defined]

    @classmethod
    def from_weakref(cls, cdata):
        instance = cls.__new__(cls)
        instance.cdata = cdata
        instance._free_weak_ref = xcelerate.Storage._free_weak_ref  # type: ignore[attr-defined]
        return instance

    def expired(self):
        return xcelerate.Storage._expired(self.cdata)  # type: ignore[attr-defined]
...
def rebuild_event(device, handle):
    return xcelerate.cuda.Event.from_ipc_handle(device, handle)
...
def rebuild_tensor(cls, storage, metadata):
    ...
    t = xcelerate._utils._rebuild_tensor(storage, storage_offset, size, stride)
    if cls == xcelerate.nn.parameter.Parameter:
        ...
        t = xcelerate.nn.parameter.Parameter(t, requires_grad=requires_grad)
    else:
...
def rebuild_cuda_tensor(
    ...
    _storage = (
        storage
        if isinstance(storage, xcelerate.UntypedStorage)
        else storage._untyped_storage
    )

    t = xcelerate._utils._rebuild_tensor(
        xcelerate.storage.TypedStorage(wrap_storage=_storage, dtype=dtype, _internal=True),
        tensor_offset,
        tensor_size,
        tensor_stride,
    )

    if tensor_cls == xcelerate.nn.parameter.Parameter:
...
def reduce_tensor(tensor):
    ...
    xcelerate.utils.hooks.warn_if_has_hooks(tensor)
    ...
    #    allocator block.  BAD!
    #
    # By the way, in old versions of Pyxcelerate, we supported this situation
    ...
    # https://github.com/pyxcelerate/pyxcelerate/issues/110543
    from xcelerate.nested._internal.nested_tensor import NestedTensor

    if tensor.is_nested and not isinstance(tensor, NestedTensor):
        return reduce_nested_tensor(tensor)

    if tensor.layout in {
        xcelerate.sparse_coo,
        xcelerate.sparse_csr,
        xcelerate.sparse_bsr,
        xcelerate.sparse_csc,
        xcelerate.sparse_bsc,
    }:
...
def rebuild_nested_tensor(
    ...
    return xcelerate._nested_view_from_buffer_copy(buffer, sizes, strides, offsets)
...
def rebuild_sparse_coo_tensor(
    ...
    return xcelerate.sparse_coo_tensor(indices, values, shape, is_coalesced=is_coalesced)
...
def reduce_sparse_tensor(sparse):
    if sparse.layout is xcelerate.sparse_coo:
        ...
        if sparse.layout in {xcelerate.sparse_csr, xcelerate.sparse_bsr}:
            compressed_indices = sparse.crow_indices()
            plain_indices = sparse.col_indices()
        elif sparse.layout in {xcelerate.sparse_csc, xcelerate.sparse_bsc}:
...
def storage_from_cache(cls, key):
    ...
    if storage_ref is None:
        return None
    return xcelerate.UntypedStorage._new_with_weak_ptr(storage_ref.cdata)
...
def rebuild_storage_filename(cls, manager, handle, size, dtype=None):
    storage: Union[xcelerate.TypedStorage, xcelerate.UntypedStorage] = storage_from_cache(
        cls, handle
    )
    if storage is not None:
        return storage._shared_decref()
    if dtype is None:
        storage = xcelerate.UntypedStorage._new_shared_filename_cpu(manager, handle, size)
    else:
        byte_size = size * xcelerate._utils._element_size(dtype)
        untyped_storage: xcelerate.UntypedStorage = (
            xcelerate.UntypedStorage._new_shared_filename_cpu(manager, handle, byte_size)
        )
        storage = xcelerate.TypedStorage(
...
def rebuild_typed_storage(storage, dtype):
    return xcelerate.storage.TypedStorage(wrap_storage=storage, dtype=dtype, _internal=True)


# Use for xcelerate.storage.TypedStorage
...
# Use for child classes of xcelerate.storage.TypedStorage, like xcelerate.FloatStorage
...
def reduce_storage(storage):
    ...
    if storage.is_cuda:
        ...
        if isinstance(storage, xcelerate.TypedStorage):
...
def init_reductions():
    ForkingPickler.register(xcelerate.cuda.Event, reduce_event)

    for t in xcelerate._storage_classes:
        if t.__name__ == "UntypedStorage":
            ForkingPickler.register(t, reduce_storage)
        else:
            ForkingPickler.register(t, reduce_typed_storage_child)

    ForkingPickler.register(xcelerate.storage.TypedStorage, reduce_typed_storage)

    for t in xcelerate._tensor_classes:
        ForkingPickler.register(t, reduce_tensor)

    # TODO: Maybe this should be in tensor_classes? :)
    ForkingPickler.register(xcelerate.Tensor, reduce_tensor)
    ForkingPickler.register(xcelerate.nn.parameter.Parameter, reduce_tensor)
