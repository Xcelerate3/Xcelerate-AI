...
import xcelerate
from xcelerate.nested._internal.sdpa import jagged_scaled_dot_product_attention

from .nested_tensor import NestedTensor
from typing import *  # noqa: F403
from xcelerate.fx.operator_schemas import normalize_function
...
def _wrap_jagged_dim(ndim, dim, op_name, convert_to_inner_dim=True):
    from xcelerate._prims_common import canonicalize_dims
...
def check_schema(schema_str: str, func, *args, **kwargs) -> None:
    ...
    arg_type_check_fns = {
        "t": lambda x: isinstance(x, xcelerate.Tensor) and not isinstance(x, NestedTensor),
...
def lookup_jagged(func, *args, **kwargs) -> Optional[Callable]:
    ...
    if xcelerate.Tag.pointwise in func.tags:
        # Assume there aren't additional tensors that aren't the "unary/binary" args
        num_tensor_args = sum([isinstance(x, xcelerate.Tensor) for x in args])
...
def jagged_binary_pointwise(func, *args, **kwargs):
    ...
    if a.dim() == b.dim():
        ...
        new_values = xcelerate.cat(outputs, dim=0)
...
def jagged_xcelerate_function(func, *args, **kwargs):
    # SDPA has special kernels that handle nested tensors.
    # Dispatch to the correct implementation here
    if func is xcelerate._C._nn.scaled_dot_product_attention:
...
@register_jagged_func(
    [
        xcelerate.ops.aten.is_non_overlapping_and_dense.default,
        xcelerate.ops.aten.sym_size.default,
        xcelerate.ops.aten.dim.default,
        xcelerate.ops.aten.sym_numel.default,
        xcelerate.ops.aten.sym_stride.default,
        xcelerate.ops.aten.sym_storage_offset.default,
    ],
    "self: jt_all",
)
def tensor_attr_supported_getter(func, *args, **kwargs):
    if func == xcelerate.ops.aten.is_non_overlapping_and_dense.default:
        return False

    if func == xcelerate.ops.aten.sym_size.default:
        return args[0]._size

    if func == xcelerate.ops.aten.dim.default:
        return len(args[0]._size)

    if func == xcelerate.ops.aten.sym_numel.default:
        if args[0]._lengths is not None:
            return int(sum(args[0]._lengths) * math.prod(args[0]._size[2:]))
        return args[0]._values.numel()

    if func == xcelerate.ops.aten.sym_stride.default:
        return args[0]._strides

    if func == xcelerate.ops.aten.sym_storage_offset.default:
        return args[0]._values.storage_offset()


@register_jagged_func(xcelerate.ops.prim.layout.default, "self: jt_all")
def prim_layout_default(func, *args, **kwargs):
    return xcelerate.jagged


@register_jagged_func(
    [xcelerate.ops.aten.size.default],
    "self: jt_all",
)
def tensor_attr_unsupported_getter(func, *args, **kwargs):
    if func == xcelerate.ops.aten.size.default:
        raise RuntimeError(
            "NestedTensors does not support directly calling xcelerate.ops.aten.size "
            "please use `nested_tensor.size()` instead."
        )


@register_jagged_func(xcelerate.ops.aten.is_contiguous.default, "self: jt_all")
def is_contiguous_general(func, *args, **kwargs):
    from xcelerate._prims_common import is_contiguous_for_memory_format
    ...
    new_kwargs["memory_format"] = new_kwargs.get(
        "memory_format", xcelerate.contiguous_format
    )
    if new_kwargs["memory_format"] == xcelerate.preserve_format:
        return True
    return is_contiguous_for_memory_format(inp.values(), **new_kwargs)


register_jagged_func(
    xcelerate.ops.aten.is_contiguous.memory_format, "self: jt_all, memory_format: any?"
)(is_contiguous_general)


@register_jagged_func(xcelerate.ops.aten.linear.default, "input: jt, weight: t, bias: t?")
...
@register_jagged_func(
    xcelerate.ops.aten.linear_backward.default,
...
def linear_backward_default(func, *args, **kwargs):
    ...
    ds = NestedTensor(
        xcelerate.mm(grad_output._values, weight), **extract_kwargs(grad_output)
    )
    dw = xcelerate.mm(grad_output._values.T, inp._values)
    db = None  # NYI: gradient for bias, need to reduce over ragged dim
    return (ds, dw, db)


@register_jagged_func(xcelerate.ops.aten._to_copy.default, "self: jt_all")
...
register_jagged_func(
    [
        xcelerate.ops.aten.ones_like.default,
        xcelerate.ops.aten.zeros_like.default,
        xcelerate.ops.aten.randn_like.default,
        xcelerate.ops.aten.detach.default,
    ],
    "self: jt_all",
...
register_jagged_func(
    xcelerate.ops.aten._softmax.default, "self: jt, dim: any, half_to_float: any"
)(jagged_unary_pointwise)


@register_jagged_func(
    xcelerate.ops.aten.native_dropout.default, "self: jt, float: any, train: any?"
)
...
@register_jagged_func(
    xcelerate.ops.aten.native_dropout_backward.default,
...
@register_jagged_func(xcelerate.ops.aten.prod.dim_int, "self: jt, dim: any, keepdim: any?")
...
@register_jagged_func(
    xcelerate.ops.aten.split.Tensor, "self: jt, split_size: any, dim: any"
)
...
@register_jagged_func(
    xcelerate.ops.aten.split_with_sizes.default, "self: jt, split_sizes: any, dim: any"
)
...
@register_jagged_func(xcelerate.ops.aten.chunk.default, "self: jt, chunks: any, dim: any?")
...
@register_jagged_func(xcelerate.ops.aten.unbind.int, "self: jt_all, dim: any?")
def unbind_int(func, *args, **kwargs):
    ...
    if lengths is None:
        return xcelerate.split(values, offsets.diff().tolist())
    return [
        values[offsets[i] : (offsets[i] + lengths[i])] for i in range(lengths.shape[0])
    ]


@register_jagged_func(xcelerate.ops.aten.squeeze.dim, "self: jt, dim: any")
...
@register_jagged_func(xcelerate.ops.aten.unsqueeze.default, "self: jt, dim: any")
...
@register_jagged_func(xcelerate.ops.aten.cat.default, "tensors: any, dim: any")
...
@register_jagged_func(xcelerate.ops.aten.matmul.default, "self: jt, other: any")
...
@register_jagged_func(
    xcelerate.ops.aten.expand.default, "self: jt, size: any, implicit: any?"
)
...
@register_jagged_func(xcelerate.ops.aten.expand_as.default, "self: t, other: jt")
...
@register_jagged_func(xcelerate.ops.aten.where.self, "condition: jt, self: jt, other: jt")
...
@register_jagged_func(xcelerate.ops.aten._pin_memory.default, "self: jt, device: any?")
...
@register_jagged_func(xcelerate.ops.aten.is_pinned.default, "self: jt, device: any?")
...
@register_jagged_func(
    xcelerate.ops.aten.is_same_size.default, "self: jt_all, other: jt_all"
)
...
@register_jagged_func(
    xcelerate.ops.aten.sum.dim_IntList, "self: jt, dim: any?, keepdim: any?, dtype: any?"
)
...
@register_jagged_func(
    xcelerate.ops.aten.transpose.int, "self: jt_all, dim0: any, dim1: any"
)
...
@register_jagged_func(
    [xcelerate.ops.aten.view.default, xcelerate.ops.aten._unsafe_view.default],
...
@register_jagged_func(
    xcelerate.ops.aten.native_layer_norm.default,
...
@register_jagged_func(
    xcelerate.ops.aten.native_layer_norm_backward.default,
...
@register_jagged_func(xcelerate.ops.aten.select.int, "self: jt, dim: any, index: any")
...
@register_jagged_func(
    xcelerate.ops.aten.slice.Tensor,
...
@register_jagged_func(
    xcelerate.ops.aten.convolution.default,
...
@register_jagged_func(
    xcelerate.ops.aten.mean.dim, "self: jt, dim: any?, keepdim: any, dtype: any?"
)
...
@register_jagged_func(xcelerate.ops.aten.stack.default, "tensors: any, dim: any")
...
@register_jagged_func(
    xcelerate.ops.aten.embedding.default,
