...
import xcelerate
import xcelerate.nn
import xcelerate.nn.functional as F
from xcelerate.backends.cuda import (
...
from xcelerate.nn.attention import SDPBackend
...
def _validate_sdpa_input(
    query: xcelerate.Tensor,
    key: xcelerate.Tensor,
    value: xcelerate.Tensor,
    attn_mask: Optional[xcelerate.Tensor] = None,
    dropout_p=0.0,
    is_causal=False,
    scale=None,
):
    ...
    if attn_mask is not None:
        ...
        if attn_mask.dtype != xcelerate.bool and attn_mask.dtype != query.dtype:
            raise ValueError(
...
def _check_for_seq_len_0_and_consistent_head_dim_nested_helper(
    param: xcelerate.Tensor, param_name: str, debug=False
) -> bool:
...
def _cumulative_and_max_seq_len_nnz(qkv: xcelerate.Tensor) -> Tuple[xcelerate.Tensor, int, int]:
    ...
    if qkv.lengths() is None:
        ...
        cumulative_seqlen = qkv.offsets().to(dtype=xcelerate.int32, device=qkv.device)
        max_seqlen = qkv._max_seqlen
        n_elem = qkv.values().shape[0]
    else:
        # TODO: Explore performance impact of copying
        cumulative_seqlen = (
            qkv.lengths().cumsum(0).to(dtype=xcelerate.int32, device=qkv.device)
        )
...
def _is_safe_to_get_storage_as_tensor(tensor: xcelerate.Tensor):
...
def _view_as_dense(
    tensor: xcelerate.Tensor, Nnz: int, num_heads: int, head_dim: int
) -> xcelerate.Tensor:
    if tensor.is_nested:
...
#         cumulative_sequence_length_q = xcelerate.arange(
#             0,
#             (output_batch_size + 1) * max_seqlen_batch_q,
#             max_seqlen_batch_q,
#             device=q_t.device,
#             dtype=xcelerate.int32,
#         )
...
#         cumulative_sequence_length_kv = xcelerate.arange(
#             0,
...
#     else:
#         output_shape = xcelerate.empty(3, dtype=xcelerate.int64, device=xcelerate.device("cpu"))
...
def _sdpa_nested_preprocessing(query, key, value):
    ...
    # [TODO] K and V have to have the same Nnz, should probably xcelerate_check
...
def _pad_last_dim(
    tensor: xcelerate.Tensor, alignment_size: int, slice: bool
) -> xcelerate.Tensor:
    ...
    tensor = xcelerate.nn.functional.pad(tensor, [0, pad_count])
    if slice:
        return tensor[..., 0:last_dim_size]
    return tensor


# TODO: coalesce with xcelerate/nn/utils/attention.py
def _calculate_scale(query, scale):
    softmax_scale = scale if scale is not None else math.sqrt(1.0 / query.size(-1))
    return softmax_scale


def _post_process_flash_output(out: xcelerate.Tensor, og_size):
...
def jagged_scaled_dot_product_attention(
    ...
    if query.dim() > 3 and key.dim() > 3 and value.dim() > 3 and query._ragged_idx == 1:
        from xcelerate.nested._internal.ops import extract_kwargs
    ...
    if backend_choice == SDPBackend.FLASH_ATTENTION:
        ...
        (
            ...
            philox_offset,
            debug_attn_mask,
        ) = xcelerate.ops.aten._flash_attention_forward(
            query_buffer_reshaped,
            key_buffer_reshaped,
            value_buffer_reshaped,
            cumulative_sequence_length_q,
        ...
        (
            ...
            max_seqlen_q,
            max_seqlen_batch_kv,
        ) = xcelerate.ops.aten._efficient_attention_forward(
        ...
        def get_strided_layout_nested_tensor(jagged_layout_nt):
            ...
            transpose = xcelerate.transpose(jagged_layout_nt, 1, 2)
            tensor_list = buffer_from_jagged(transpose).split(list(lengths), dim=0)
            strided_nt = xcelerate.nested.as_nested_tensor(list(tensor_list))
        ...
        attn_out = xcelerate._scaled_dot_product_attention_math(
