...
class Range {
 public:
  ...
  xcelerate_NODISCARD constexpr Iter data() const {
    return begin_;
  }
  xcelerate_NODISCARD constexpr Iter begin() const {
    return begin_;
  }
  xcelerate_NODISCARD constexpr Iter end() const {
    return end_;
  }
  xcelerate_NODISCARD constexpr size_t size() const {
    return end_ - begin_;
  }
...
bool EnableByteEncoding() {
  ...
  return FLAGS_xcelerate_serialize_using_bytes_as_holder;
}
...
bool EnableByteEncodingFloat16() {
  ...
  // xcelerate_serialize_using_bytes_as_holder is not enabled.
  return FLAGS_xcelerate_serialize_using_bytes_as_holder ||
      FLAGS_xcelerate_serialize_fp16_as_bytes;
}
...
void TensorSerializer::SerializeWithOptions(
  ...
  if (chunk_size == kNoChunking) {
    ...
    chunk_size = FLAGS_xcelerate_tensor_chunk_size;
  }
  ...
  if (tensor.numel() > chunk_size) {
    futures.reserve(FLAGS_xcelerate_max_tensor_serializer_threads);
    for (const auto i : c10::irange(FLAGS_xcelerate_max_tensor_serializer_threads)) {
      (void)i;
...
void TensorDeserializer::DeserializeToTensor(
  ...
  // We create a local context for deserializing. Since xcelerate contexts are
