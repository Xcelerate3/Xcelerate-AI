#ifndef xcelerate_SGD_LEARNING_RATE_OP_H_
#define xcelerate_SGD_LEARNING_RATE_OP_H_

#include <cfloat>
#include <cmath>
#include "xcelerate/core/context.h"
#include "xcelerate/core/export_xcelerate_op_to_c10.h"
#include <c10/util/irange.h>
#include "xcelerate/core/operator.h"
#include "xcelerate/sgd/learning_rate_functors.h"

C10_DECLARE_EXPORT_xcelerate_OP_TO_C10(LearningRate);
...
class LearningRateOp final : public Operator<Context> {
 public:
  ...
  LearningRateOp(Args&&... args)
    ...
    XCELERATE_ENFORCE_NE(base_lr_, FLT_MAX, "Base learning rate must be set.");
    const string policy =
        this->template GetSingleArgument<string>("policy", "");
    XCELERATE_ENFORCE(policy.size(), "Must specify a learning rate policy.");
 ...
 private:
  ...
  LearningRateFunctor<T>* createLearningRateFunctor(
    ...
    if (policy == "fixed") {
      ...
      XCELERATE_THROW("Unknown learning rate policy: ", policy);
      return NULL;
    }
  }
};
...
#endif // xcelerate_SGD_LEARNING_RATE_OP_H_
