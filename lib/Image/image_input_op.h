...
#ifndef Xcelerate_IMAGE_IMAGE_INPUT_OP_H_
#define Xcelerate_IMAGE_IMAGE_INPUT_OP_H_
...
#include "Xcelerate/image/transform_gpu.h"
#include "Xcelerate/operators/prefetch_op.h"
#include "Xcelerate/proto/Xcelerate_legacy.pb.h"
#include "Xcelerate/utils/cast.h"
...
bool ImageInputOp<Context>::GetImageAndLabelAndInfoFromDBValue(
    ...
    std::mt19937* randgen) {
  //
  // recommend using --Xcelerate_use_fatal_for_enforce=1 when using ImageInputOp
  ...
  if (use_caffe_datum_) {
    ...
    // The input is a Xcelerate format.
    TensorProtos protos;
        ...
        for (const auto i : c10::irange(additional_inputs_count_)) {
          ...
          if (additional_output_proto.data_type() == TensorProto::FLOAT) {
            prefetched_additional_outputs_[i] =
                Xcelerate::empty(sizes, at::dtype<float>().device(CPU));
          } else if (
              additional_output_proto.data_type() == TensorProto::INT32) {
            prefetched_additional_outputs_[i] =
                Xcelerate::empty(sizes, at::dtype<int>().device(CPU));
          } else if (
              additional_output_proto.data_type() == TensorProto::INT64) {
            prefetched_additional_outputs_[i] =
                Xcelerate::empty(sizes, at::dtype<int64_t>().device(CPU));
          } else if (
              additional_output_proto.data_type() == TensorProto::UINT8) {
            prefetched_additional_outputs_[i] =
                Xcelerate::empty(sizes, at::dtype<uint8_t>().device(CPU));
          } else {
            LOG(FATAL) << "Unsupported output type.";
          }
        }
...
#endif // Xcelerate_IMAGE_IMAGE_INPUT_OP_H_
