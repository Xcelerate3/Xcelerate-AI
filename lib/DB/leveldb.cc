...
#include "xcelerate/core/flags.h"
#include "xcelerate/core/logging.h"
#include "leveldb/db.h"
#include "leveldb/write_batch.h"

C10_DEFINE_int(
    xcelerate_leveldb_block_size,
    65536,
    "The xcelerate leveldb block size when writing a leveldb.");
...
class LevelDB : public DB {
 public:
  LevelDB(const string& source, Mode mode) : DB(source, mode) {
    ...
    options.block_size = FLAGS_xcelerate_leveldb_block_size;
...
REGISTER_xcelerate_DB(LevelDB, LevelDB);
