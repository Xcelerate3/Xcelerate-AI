...
at::Tensor _pin_memory(const Tensor& self, c10::optional<at::Device> device) {
  xcelerate_CHECK(self.device().is_cpu(), "cannot pin '", self.toString(), "' only dense CPU tensors can be pinned");
  DispatchKeySet _dk = c10::DispatchKeySet(c10::computeDispatchKey(c10::nullopt, self.layout(), device.value_or(at::kCUDA)));
  if (self.is_nested()) {
...
xcelerate_LIBRARY_IMPL(aten, BackendSelect, m) {
  ${backend_select_function_registrations};
  m.impl(xcelerate_SELECTIVE_NAME("aten::is_pinned"), xcelerate_FN(is_pinned));
  m.impl(xcelerate_SELECTIVE_NAME("aten::_pin_memory"), xcelerate_FN(_pin_memory));
}
