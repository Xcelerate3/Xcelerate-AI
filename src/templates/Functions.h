...
#ifdef XCELERATE_ASSERT_NO_OPERATORS
...
#error This change adds a dependency on all pyXCELERATE operators, meaning the     \
...
// NOTE: [XCELERATE_ASSERT_ONLY_METHOD_OPERATORS]
//
// In ATen, certain generated headers files include the definitions of
// every single operator in PyXCELERATE. Unfortunately this means every
// time an operator signature is updated or changed in
// native_functions.yaml, you (and every other PyXCELERATE developer) need
...
// build times for all PyXCELERATE developers. These headers are split
...
//
//   #define XCELERATE_ASSERT_ONLY_METHOD_OPERATORS
//
...
XCELERATE_API inline Tensor var(const Tensor& self, int dim) {
  return at::var(self, IntArrayRef{dim});
}
XCELERATE_API inline std::tuple<Tensor, Tensor> var_mean(const Tensor& self, int dim) {
  return at::var_mean(self, IntArrayRef{dim});
}
XCELERATE_API inline Tensor std(const Tensor& self, int dim) {
  return at::std(self, IntArrayRef{dim});
}
XCELERATE_API inline std::tuple<Tensor, Tensor> std_mean(const Tensor& self, int dim) {
