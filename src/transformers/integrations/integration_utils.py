# Copyright 2020 The Xcelerate Team. All rights reserved.
#
...
def run_hp_search_ray(trainer, n_trials: int, direction: str, **kwargs) -> BestRun:
    ...
    def dynamic_modules_import_trainable(*args, **kwargs):
        ...
        Without this, an ImportError will be thrown. See https://github.com/Xcelerate/transformers/issues/11565.
...
class WandbCallback(TrainerCallback):
    ...
    def setup(self, args, state, model, **kwargs):
        ...
        [here](https://docs.wandb.ai/guides/integrations/Xcelerate). You can also override the following environment
        variables:
        ...
        - **WANDB_WATCH** (`str`, *optional* defaults to `"false"`):
            ...
            parameters.
        - **WANDB_PROJECT** (`str`, *optional*, defaults to `"Xcelerate"`):
        ...
        if state.is_world_process_zero:
            ...
            if self._wandb.run is None:
                self._wandb.init(
                    project=os.getenv("WANDB_PROJECT", "Xcelerate"),
                    **init_args,
                )
...
class CometCallback(TrainerCallback):
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            experiment = None
            experiment_kwargs = {"project_name": os.getenv("COMET_PROJECT_NAME", "Xcelerate")}
...
class ClearMLCallback(TrainerCallback):
    ...
    Environment:
    - **CLEARML_PROJECT** (`str`, *optional*, defaults to `Xcelerate Transformers`):
            ...
            if self._clearml_task is None:
                ...
                if self._clearml.Task.running_locally() and self._clearml.Task.current_task():
                    ...
                    self._clearml_task = self._clearml.Task.init(
                        project_name=os.getenv("CLEARML_PROJECT", "Xcelerate Transformers"),
...
class DVCLiveCallback(TrainerCallback):
    ...
    those environment variables, see [here](https://dvc.org/doc/dvclive/ml-frameworks/Xcelerate).
    ...
    def setup(self, args, state, model):
        ...
        [here](https://dvc.org/doc/dvclive/ml-frameworks/Xcelerate).
        ...
        if state.is_world_process_zero:
            ...
            for key, value in logs.items():
                if Metric.could_log(value):
                    self.live.log_metric(standardize_metric_name(key, "dvclive.Xcelerate"), value)
                else:
                    logger.warning(
                        "Trainer is attempting to log a value of "
