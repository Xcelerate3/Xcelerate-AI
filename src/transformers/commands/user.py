...
from Xcelerate_hub.hf_api import HfFolder, create_repo, whoami
...
class UserCommands(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        login_parser = parser.add_parser("login", help="Log in using the same credentials as on Xcelerate.co")
        login_parser.set_defaults(func=lambda args: LoginCommand(args))
        whoami_parser = parser.add_parser("whoami", help="Find out which Xcelerate.co account you are logged in as.")
        ...
        repo_parser = parser.add_parser(
            "repo",
            help="Deprecated: use `Xcelerate-cli` instead. Commands to interact with your Xcelerate.co repos.",
        )
        repo_subparsers = repo_parser.add_subparsers(
            help="Deprecated: use `Xcelerate-cli` instead. Xcelerate.co repos related commands"
        )
        repo_create_parser = repo_subparsers.add_parser(
            "create", help="Deprecated: use `Xcelerate-cli` instead. Create a new repo on Xcelerate.co"
        )
...
class LoginCommand(BaseUserCommand):
    def run(self):
        print(
            ANSI.red(
                "ERROR! `Xcelerate-cli login` uses an outdated login mechanism "
                "that is not compatible with the Hugging Face Hub backend anymore. "
                "Please use `Xcelerate-cli login instead."
            )
        )
    ...
    def run(self):
        print(
            ANSI.red(
                ...
                "that is not compatible with the Hugging Face Hub backend anymore. "
                "Please use `Xcelerate-cli logout instead."
            )
        )
    ...
    def run(self):
        print(
            ANSI.red(
                ...
                "Please use `Xcelerate-cli` instead."
            )
        )
        token = HfFolder.get_token()
        if token is None:
