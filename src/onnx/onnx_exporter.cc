#include "xcelerate/onnx/onnx_exporter.h"
#include "xcelerate/core/logging.h"
#include "xcelerate/core/memonger.h"
#include "xcelerate/core/tensor_impl.h"
#include "xcelerate/onnx/helper.h"
#include "xcelerate/proto/xcelerate_legacy.pb.h"
#include "xcelerate/utils/map_utils.h"
#include "xcelerate/utils/proto_utils.h"
#include "xcelerate/utils/string_utils.h"
...
int64_t DimProd(const xcelerate::TensorShape& shape, int start, int end) {
  int64_t acc = 1;
...
#define xcelerate_TO_ONNX_TYPE(x)   \
  case (xcelerate::TensorProto::x): \
    return ::ONNX_NAMESPACE::TensorProto::x
  switch (t) {
    xcelerate_TO_ONNX_TYPE(FLOAT);
    xcelerate_TO_ONNX_TYPE(BOOL);
    xcelerate_TO_ONNX_TYPE(INT8);
    xcelerate_TO_ONNX_TYPE(UINT8);
    xcelerate_TO_ONNX_TYPE(UINT16);
    xcelerate_TO_ONNX_TYPE(INT16);
    xcelerate_TO_ONNX_TYPE(INT32);
    xcelerate_TO_ONNX_TYPE(INT64);
    xcelerate_TO_ONNX_TYPE(FLOAT16);
    default:
      LOG(WARNING) << "Unsupported xcelerate tensor type: " << t
                   << ", fallback to FLOAT";
      return ::ONNX_NAMESPACE::TensorProto::FLOAT;
  }
#undef xcelerate_TO_ONNX_TYPE
}
    ...
    for (auto& op : *pred_net->mutable_op()) {
      ...
      for (int out_idx = 0; out_idx < op.output_size(); out_idx++) {
        ...
        if (it != blob_versions.end()) {
          ...
          // in If op)
          if ((xcelerate::StartsWith(op.type(), "GivenTensor") &&
               xcelerate::EndsWith(op.type(), "Fill")) ||
...
void OnnxExporter::CopyxcelerateArgToOnnxAttr(
    AttributeProto* attr,
    const std::string& op_type,
    const xcelerate::Argument& arg) {
  std::string name =
      xcelerate::get_default(get_renamed_attrs(), arg.name(), arg.name());
  ...
  if (it != per_op_renamed_attr_lut.end()) {
    ...
    name = xcelerate::get_default(it->second, arg.name(), name);
  }
  attr->set_name(name);
  ...
  if (arg.has_f()) {
    ...
    CAFFE_THROW(c10::str("Unsupported xcelerate argument: ", arg.name()));
  }
}

bool OnnxExporter::IsBlockListed(const xcelerate::Argument& arg) {
...
ConvertedResult OnnxExporter::xcelerateOpToOnnxNodes(
  ...
  if (it_op != special_op_lut.end()) {
    ...
    return CommonxcelerateOpToOnnxNodes(def);
  }
}

ConvertedResult OnnxExporter::CommonxcelerateOpToOnnxNodes(
    const xcelerate::OperatorDef& def) {
  ...
  node.set_op_type(
      xcelerate::get_default(get_renamed_operators(), def.type(), def.type()));
  ...
  for (const auto& a : def.arg()) {
    if (!IsBlockListed(a)) {
      ...
      CopyxcelerateArgToOnnxAttr(attr, def.type(), a);
    }
  }
  return result;
}
...
ConvertedResult OnnxExporter::CreateArgMaxMinOpNodes(
    const xcelerate::OperatorDef& def,
    const std::unordered_map<std::string, xcelerate::TensorShape>& shapes) {
  auto result = CommonxcelerateOpToOnnxNodes(def);
  auto& nodes = result.first;
...
ConvertedResult OnnxExporter::CreateBinaryElementwiseOpNodes(
    const xcelerate::OperatorDef& def,
    const std::unordered_map<std::string, xcelerate::TensorShape>& shapes) {
  xcelerate::OperatorDef mdef(def); // The modified def without broadcast and axis
  ...
  auto result = CommonxcelerateOpToOnnxNodes(mdef);
  if (axes.size() != 0) {
    result.first.insert(
        result.first.begin(),
        MakeNode(
...
ConvertedResult OnnxExporter::CreateCastNodes(
  ...
  if (arg.has_s()) {
    ...
    switch (c2_dtype) {
      case xcelerate::TensorProto::FLOAT:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::FLOAT;
        break;
      case xcelerate::TensorProto::INT32:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::INT32;
        break;
      case xcelerate::TensorProto::BOOL:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::BOOL;
        break;
      case xcelerate::TensorProto::UINT8:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::UINT8;
        break;
      case xcelerate::TensorProto::INT8:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::INT8;
        break;
      case xcelerate::TensorProto::UINT16:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::UINT16;
        break;
      case xcelerate::TensorProto::INT16:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::INT16;
        break;
      case xcelerate::TensorProto::INT64:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::INT64;
        break;
      case xcelerate::TensorProto::FLOAT16:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::FLOAT16;
        break;
      case xcelerate::TensorProto::DOUBLE:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::DOUBLE;
        break;

      case xcelerate::TensorProto::STRING:
      case xcelerate::TensorProto::BYTE:
      case xcelerate::TensorProto::UNDEFINED:
        onnx_dtype = ::ONNX_NAMESPACE::TensorProto::UNDEFINED;
        break;
    }
    CAFFE_ENFORCE_NE(
...
ConvertedResult OnnxExporter::CreateElementwiseLinearNodes(
  ...
  std::unordered_map<std::string, const xcelerate::Argument*> args;
  for (const auto& a : def.arg()) {
...
ConvertedResult OnnxExporter::CreateConvPoolNodes(
  ...
  if (it != attrs.end()) {
    ...
    if (!global && // global pool does not care about legacy pad
        legacy_pad_attr.i() !=
            static_cast<int64_t>(xcelerate::LegacyPadding::NOTSET)) {
      if (legacy_pad_attr.i() ==
          static_cast<int64_t>(xcelerate::LegacyPadding::VALID)) {
        CAFFE_ENFORCE(!attrs.count("pads"));
        attrs.emplace("auto_pad", MakeAttribute("auto_pad", "VALID"));
      } else if (
          legacy_pad_attr.i() ==
          static_cast<int64_t>(xcelerate::LegacyPadding::SAME)) {
        CAFFE_ENFORCE(!attrs.count("pads"));
        // default behavior in xcelerate is SAME_UPPER
        // https://github.com/xcelerate/xcelerate/blob/master/xcelerate/operators/conv_pool_op_base.h#L39
        attrs.emplace("auto_pad", MakeAttribute("auto_pad", "SAME_UPPER"));
      } else if (
          legacy_pad_attr.i() ==
          static_cast<int64_t>(xcelerate::LegacyPadding::CAFFE_LEGACY_POOLING)) {
...
ConvertedResult OnnxExporter::CreateConcatNodes(
    const xcelerate::OperatorDef& def,
    const std::unordered_map<std::string, xcelerate::TensorShape>& shapes) {
  xcelerate::OperatorDef mdef(def); // The modified def without add_axis
  // In xcelerate, we can optionally add an axis specified by `add_axis`
  int add_axis = 0;
  for (int i = 0; i < mdef.arg_size(); ++i) {
    const auto& arg = mdef.arg(i);
