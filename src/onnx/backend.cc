#include "xcelerate/onnx/backend.h"
...
#include "xcelerate/utils/map_utils.h"
...
bool IsOperator(const std::string& op_type) {
  ...
  static std::set<std::string>* ops_ =
      new std::set<std::string>(xcelerate::GetRegisteredOperators());
  return ops_->count(xcelerate::OpRegistryKey(op_type, "DEFAULT"));
}

xcelerate::DeviceOption GetDeviceOption(const Device& onnx_device) {
  static const std::unordered_map<DeviceType, xcelerate::DeviceType> m = {
      {DeviceType::CPU, xcelerate::DeviceType::CPU},
      {DeviceType::CUDA, xcelerate::DeviceType::CUDA}};
  xcelerate::DeviceOption d;
...
void BuildOperator(
    ...
    const std::vector<xcelerate::Argument>& args) {
  c2_op->set_name("");
...
void BuildOperator(
    ...
    const std::vector<std::string>& inputs,
    const std::vector<std::string>& outputs) {
  std::vector<xcelerate::Argument> empty;
  BuildOperator(c2_op, op_type, inputs, outputs, empty);
}

void CopyOnnxAttrValueToxcelerateArg(
    xcelerate::Argument* arg,
...
::google::protobuf::RepeatedPtrField<xcelerate::Argument>
OnnxAttributes::OnnxAttrToxcelerateArg(
    std::function<std::string(const std::string&)> mapper) const {
  ::google::protobuf::RepeatedPtrField<xcelerate::Argument> args;
  for (const auto& kv : onnx_attrs_) {
    ...
    CopyOnnxAttrValueToxcelerateArg(arg, attr);
  }
...
xcelerateBackend::get_broken_operators() const {
...
const std::unordered_set<std::string>& xcelerateBackend::get_rnn_operators()
    const {
...
// Operators that are different between xcelerate and
...
xcelerateBackend::get_renamed_operators() const {
  const static std::unordered_map<std::string, std::string> kRenamedOperators{
      {"xcelerateConvTranspose", "ConvTranspose"},
...
const std::unordered_map<std::string, std::string>&
xcelerateBackend::get_renamed_attrs() const {
...
const std::
    ...
    xcelerateBackend::get_per_op_renamed_attrs() const {
  const static std::
      unordered_map<std::string, std::unordered_map<std::string, std::string>>
          kPerOpRenamedAttrs = {
              {"Squeeze", {{"axes", "dims"}}},
...
// function from ToffeIR node_def to xcelerate op_def
const std::unordered_map<std::string, xcelerateBackend::SpecialOpConverter>&
xcelerateBackend::get_special_operators() const {
  const static std::
      unordered_map<std::string, xcelerateBackend::SpecialOpConverter>
          kSpecialOperators = {
              {"ArgMax", &xcelerateBackend::CreateArgMaxMin},
              {"ArgMin", &xcelerateBackend::CreateArgMaxMin},
              {"Cast", &xcelerateBackend::CreateCast},
              {"Constant", &xcelerateBackend::CreateConstant},
              {"ConstantOfShape", &xcelerateBackend::CreateConstantOfShape},
              {"Conv", &xcelerateBackend::CreateConvPoolOpBase},
              {"AveragePool", &xcelerateBackend::CreateConvPoolOpBase},
              {"GlobalAveragePool", &xcelerateBackend::CreateConvPoolOpBase},
              {"GlobalMaxPool", &xcelerateBackend::CreateConvPoolOpBase},
              {"MaxPool", &xcelerateBackend::CreateConvPoolOpBase},
              {"Reshape", &xcelerateBackend::CreateReshape},
              {"Int8Reshape", &xcelerateBackend::CreateReshape},
              {"Gather", &xcelerateBackend::CreateGather},
              {"Gemm", &xcelerateBackend::CreateGemm},
              {"Pad", &xcelerateBackend::CreatePad},
              {"Concat", &xcelerateBackend::CreateConcat},
              {"Int8Concat", &xcelerateBackend::CreateConcat},
              {"LogSoftmax", &xcelerateBackend::CreateLogSoftmax},
              {"Slice", &xcelerateBackend::CreateSlice},
              {"Split", &xcelerateBackend::CreateSplit},
              {"Reciprocal", &xcelerateBackend::CreateReciprocal},
              {"BatchNormalization", &xcelerateBackend::CreateBatchNormalization},
              {"MatMul", &xcelerateBackend::CreateMatMul},
              {"Upsample", &xcelerateBackend::CreateUpsample},
              {"Dropout", &xcelerateBackend::CreateDropout},
              {"LRN", &xcelerateBackend::CreateLRN},
              {"DynamicSlice", &xcelerateBackend::CreateDynamicSlice},
              {"RandomNormal", &xcelerateBackend::CreateRandomNormal},
              {"RandomNormalLike", &xcelerateBackend::CreateRandomNormal},
              {"Where", &xcelerateBackend::CreateWhereOp},
              {"NonZero", &xcelerateBackend::CreateNonZeroOp},
              {"Multinomial", &xcelerateBackend::CreateMultinomialOp}};
...
xcelerateOps xcelerateBackend::CreateArgMaxMin(
    OnnxNode* onnx_node,
  ...
  return CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
}

xcelerateOps xcelerateBackend::CreateCast(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto c2_op = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);

  auto onnx_dtype =
      onnx_node->attributes.get<int64_t>("to", TensorProto::UNDEFINED);
  auto c2_dtype = xcelerate::TensorProto::UNDEFINED;
  switch (onnx_dtype) {
    case ::ONNX_NAMESPACE::TensorProto::FLOAT:
      c2_dtype = xcelerate::TensorProto::FLOAT;
      break;
    case ::ONNX_NAMESPACE::TensorProto::UINT8:
      c2_dtype = xcelerate::TensorProto::UINT8;
      break;
    case ::ONNX_NAMESPACE::TensorProto::INT8:
      c2_dtype = xcelerate::TensorProto::INT8;
      break;
    case ::ONNX_NAMESPACE::TensorProto::UINT16:
      c2_dtype = xcelerate::TensorProto::UINT16;
      break;
    case ::ONNX_NAMESPACE::TensorProto::INT16:
      c2_dtype = xcelerate::TensorProto::INT16;
      break;
    case ::ONNX_NAMESPACE::TensorProto::INT32:
      c2_dtype = xcelerate::TensorProto::INT32;
      break;
    case ::ONNX_NAMESPACE::TensorProto::INT64:
      c2_dtype = xcelerate::TensorProto::INT64;
      break;
    case ::ONNX_NAMESPACE::TensorProto::STRING:
      c2_dtype = xcelerate::TensorProto::STRING;
      break;
    case ::ONNX_NAMESPACE::TensorProto::BOOL:
      c2_dtype = xcelerate::TensorProto::BOOL;
      break;
    case ::ONNX_NAMESPACE::TensorProto::FLOAT16:
      c2_dtype = xcelerate::TensorProto::FLOAT16;
      break;
    case ::ONNX_NAMESPACE::TensorProto::DOUBLE:
      c2_dtype = xcelerate::TensorProto::DOUBLE;
      break;
    ...
    case ::ONNX_NAMESPACE::TensorProto::UNDEFINED:
      c2_dtype = xcelerate::TensorProto::UNDEFINED;
      break;
  };

  CAFFE_ENFORCE_NE(
      c2_dtype,
      xcelerate::TensorProto::UNDEFINED,
      "Casting to '",
      onnx_dtype,
...
xcelerateOps xcelerateBackend::CreateConstant(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
...
xcelerateOps xcelerateBackend::CreateConstantOfShape(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
...
//  Note [xcelerate ConvPoolOpBase]
//  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//  To understand what is going on here, we have to talk a little bit about
//  xcelerate's internals.
//
//  First, it's important to know that all of xcelerate's pooling and convolution
...
//  sees on these operators.  Unfortunately, xcelerate's documentation generator
//  doesn't know how to handle cases like this, so for example, if you look at
//  the docs for MaxPool at
//  <https://xcelerate.ai/docs/operators-catalogue.html#maxpool> you won't see any
//  of the attributes.  You have to go source diving to find the information; in
//  particular, you want to look at:
//  https://github.com/xcelerate/xcelerate/blob/master/xcelerate/operators/conv_pool_op_base.h
//  This class handles *global* pooling as well.
//
//  Second, it's important to know what xcelerate expects for padding, which can
//  be somewhat difficult to understand from the code because xcelerate handles
...
//  *twice* the size of kernels.  So if you have a 2D convolution, xcelerate
...
//  differently.
xcelerateOps xcelerateBackend::CreateConvPoolOpBase(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  ...
  if (attributes.HasAttribute("kernel_shape") &&
    ...
    if (kernel_shape.size() == pads.size()) {
      // xcelerate requires pads to be twice the size of kernels.
...
xcelerateOps xcelerateBackend::CreateReshape(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto c2_op = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
...
xcelerateOps xcelerateBackend::CreateRandomNormal(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto& attributes = onnx_node->attributes;

  if (attributes.HasAttribute("seed")) {
    CAFFE_THROW("xcelerate GaussianFill does not support random seed");
  }

  if (attributes.HasAttribute("dtype")) {
    if (attributes.get<int64_t>("dtype") != TensorProto::FLOAT) {
      CAFFE_THROW("xcelerate GaussianFill only support FLOAT dtype");
    }
...
xcelerateOps xcelerateBackend::CreateWhereOp(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  // The native xcelerate op doesn't support broadcasting, so we defer the handling
  ...
  return CommonOnnxNodeToxcelerateOps(&new_node, ctx);
}

xcelerateOps xcelerateBackend::CreateNonZeroOp(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  // Native xcelerate doesn't support NonZero, fallback to ATen.
  ...
  attr->set_s("nonzero");
  OnnxNode new_node(converted);
  auto ret = CommonOnnxNodeToxcelerateOps(&new_node, ctx);
...
xcelerateOps xcelerateBackend::CreateMultinomialOp(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  // Fallback to ATen.
  ...
  if (onnx_dtype == ::ONNX_NAMESPACE::TensorProto::INT64) {
    ...
    xcelerate::Argument to;
    to.set_name("to");
    to.set_i(xcelerate::TensorProto::INT32);
    BuildOperator(
        c2_cast,
        "Cast",
        {c2_multinomial_output},
...
xcelerateOps xcelerateBackend::CreateReciprocal(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  const auto& node = onnx_node->node;
  if (node.input_size() != 1 || node.output_size() != 1) {
    CAFFE_THROW("xcelerate Reciprocal should have 1 input and 1 output");
  }
...
xcelerateOps xcelerateBackend::CreateGather(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  const auto& node = onnx_node->node;
  if (node.input_size() < 2 || node.output_size() < 1) {
    CAFFE_THROW("xcelerate Gather should have 2 inputs and 1 output");
  }
  ...
  if (axis == 0) {
    ...
    CAFFE_THROW(
        "xcelerate only supports Gather with axis being 0 or 1, ",
        "whereas axis is ",
        axis);
  }
...
xcelerateOps xcelerateBackend::CreateGemm(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  const auto& node = onnx_node->node;
  if (node.input_size() < 3 || node.output_size() < 1) {
    CAFFE_THROW("xcelerate Gemm should have 3 inputs and 1 output");
  }
  ...
  if (!trans_a && broadcast && check_fc()) {
    ...
    xcelerate::Argument arg_trans_a;
    arg_trans_a.set_name("trans_a");
    arg_trans_a.set_i(trans_a);
    xcelerate::Argument arg_trans_b;
    ...
    if (ctx.opset_version() >= 7) {
      ...
      xcelerate::Argument arg_broadcast;
...
xcelerateOps xcelerateBackend::CreatePad(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto& attributes = onnx_node->attributes;
  ...
  if (!(pads.size() == 8 &&
    ...
    CAFFE_THROW(
        "xcelerate only supports padding 2D Tensor, whereas padding is ", str);
  }
...
// TODO: xcelerate Concat has an extra output. It should be only
// used when doing training, so we should change xcelerate to allow
// 1 output.
xcelerateOps xcelerateBackend::CreateConcat(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto c2_op = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
  CAFFE_ENFORCE_EQ(c2_op.ops.size(), 1);
...
xcelerateOps xcelerateBackend::CreateLogSoftmax(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  const auto& node = onnx_node->node;
  if (node.input_size() < 1 || node.output_size() < 1) {
...
xcelerateOps xcelerateBackend::CreateSlice(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto op_tmp = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
  CAFFE_ENFORCE_EQ(op_tmp.ops.size(), 1);
  auto* op = op_tmp.ops.Mutable(0);
  std::unordered_map<std::string, xcelerate::Argument*> args;
  for (auto& arg : *op->mutable_arg()) {
    args.emplace(arg.name(), &arg);
  }

  xcelerate::Argument starts_vals;
  ...
  c2_op = ret.ops.Add();
  {
    xcelerate::Argument shape_starts;
  ...
  xcelerate::Argument dtype;
  dtype.set_name("dtype");
  dtype.set_i(static_cast<int64_t>(xcelerate::TensorProto::INT64));
  xcelerate::Argument constant;
  ...
  xcelerate::Argument to;
  to.set_name("to");
  to.set_i(static_cast<int64_t>(xcelerate::TensorProto::INT32));

  auto ends_vals_tensor = dummy_->NewDummyName();
  auto ends_tensor = dummy_->NewDummyName();
  c2_op = ret.ops.Add();
  {
    xcelerate::Argument shape_ends;
...
//    the behavior of xcelerate's slice operator not matching that of ONNX's slice
...
std::string xcelerateBackend::PreprocessSliceIndexTensor(
    OnnxNode* onnx_node,
    xcelerateOps& ret,
  ...
  {
    xcelerate::Argument value;
    value.set_name("value");
    value.set_i(default_value);
    xcelerate::Argument dtype;
    dtype.set_name("dtype");
    dtype.set_i(static_cast<int64_t>(xcelerate::TensorProto::INT64));
    xcelerate::Argument input_as_shape;
...
xcelerateOps xcelerateBackend::CreateDynamicSlice(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto op_tmp = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
  CAFFE_ENFORCE_EQ(op_tmp.ops.size(), 1);
  ...
  // arguments to the xcelerate Slice operator.
  std::string axes_tensor;
...
xcelerateOps xcelerateBackend::CreateBatchNormalization(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto& attributes = onnx_node->attributes;
...
xcelerateOps xcelerateBackend::CreateSplit(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto& attributes = onnx_node->attributes;
  if (!attributes.HasAttribute("axis")) {
...
xcelerateOps xcelerateBackend::CreateMatMul(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  const auto& node = onnx_node->node;
...
xcelerateOps xcelerateBackend::CreateUpsample(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto& attributes = onnx_node->attributes;
  ...
  if (ctx.opset_version() >= 7 && ctx.opset_version() < 9) {
    ...
    xcelerate::Argument arg_starts, arg_ends;
...
xcelerateOps xcelerateBackend::CreateDropout(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  if (ctx.opset_version() >= 7) {
    auto& attributes = onnx_node->attributes;
...
xcelerateOps xcelerateBackend::CreateLRN(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  auto c2_op = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
...
//==============================================
// Rest of the member functions for xcelerateBackend
//==============================================
std::unordered_set<std::string> xcelerateBackend::AllNamesInGraph(
...
//  xcelerate operators.  Besides doing a straightforward marshalling from
...
xcelerateOps xcelerateBackend::CommonOnnxNodeToxcelerateOps(
    OnnxNode* onnx_node,
    const ConversionContext& ctx) {
  xcelerateOps ret;
  auto* c2_op = ret.ops.Add();
  ...
  auto broken_version = xcelerate::get_default(
  ...
  c2_op->set_type(
      xcelerate::get_default(get_renamed_operators(), onnx_op_type, onnx_op_type));
  ...
  c2_op->mutable_arg()->MergeFrom(
      onnx_node->attributes.OnnxAttrToxcelerateArg(mapper));

  return ret;
}

xcelerateOps xcelerateBackend::ConvertNode(
  ...
  return OnnxNodeToxcelerateOps(init_model, pred_model, ctx, &onnx_node);
}

void xcelerateBackend::CheckOpSchemaArguments(
  ...
  if (schema_args.size() > 0) {
    ...
    std::transform(
        ...
        [](xcelerate::OpSchema::Argument elem) { return elem.name(); });
    ...
    VLOG(2)
        << "Operator " << op.type()
        << " does not declare arguments in its schema. Please file a xcelerate issue.";
  }
}

xcelerateOps xcelerateBackend::OnnxNodeToxcelerateOps(
  ...
  if (get_special_operators().count(onnx_node->node.op_type())) {
    ...
    res = CommonOnnxNodeToxcelerateOps(onnx_node, ctx);
  }
  ...
  for (const auto& result_op : res.ops) {
    ...
    if (schema) {
      ...
      CAFFE_THROW(
          "xcelerate has no such operator, could not find schema for ",
          result_op.type());
    }
  }
  return res;
...
void xcelerateBackend::OnnxToxcelerate(
  ...
  size_t idx_extra = 0;
  auto converter = [&](const ModelProto& model, xcelerate::NetDef* net) mutable {
    ...
    for (const auto& node : model.graph().node()) {
      ...
      if (get_rnn_operators().count(node.op_type())) {
        ...
        auto c2ops = OnnxNodeToxcelerateOps(
...
xcelerateBackendRep* xcelerateBackend::Prepare(
    const std::string& onnx_model_str,
    const std::string& device,
    const std::vector<xcelerateOps>& extras) {
  xcelerateBackendRep* rep = new xcelerateBackendRep();
  ...
  for (const auto& imp : onnx_model.opset_import()) {
    if ((!imp.has_domain()) || imp.domain().empty()) {
      ...
      if (opset_version > kKnownOpsetVersion) {
        std::cout
            << "This version of onnx-xcelerate targets ONNX operator set version "
            << kKnownOpsetVersion
  ...
  if (opset_version < 0) {
    if (onnx_model.ir_version() >= 0x00000003) {
      CAFFE_THROW(
          ...
          "version (onnx-xcelerate requires it)");
    } else {
      opset_version = 1;
    }
  }
...
template <typename T>
void ConvertIntegralValueToxcelerate(
    xcelerate::OperatorDef* c2_op,
    xcelerate::Argument* c2_values,
    const TensorProto& onnx_tensor) {
  c2_op->set_type(
...
template <>
void ConvertIntegralValueToxcelerate<::google::protobuf::int64>(
    xcelerate::OperatorDef* c2_op,
    xcelerate::Argument* c2_values,
    const TensorProto& onnx_tensor) {
...
template <>
void ConvertIntegralValueToxcelerate<::google::protobuf::uint64>(
    xcelerate::OperatorDef* c2_op,
    xcelerate::Argument* c2_values,
    const TensorProto& onnx_tensor) {
  c2_op->set_type("GivenTensorInt64Fill");
...
void xcelerateBackend::BuildTensorFillingOp(
    xcelerate::OperatorDef* c2_op,
    const TensorProto& onnx_tensor,
  ...
  if (shape_name.empty()) {
    ...
    if (onnx_tensor.data_type() == TensorProto::FLOAT) {
      ...
      ConvertIntegralValueToxcelerate<::google::protobuf::int64>(
          c2_op, c2_values, onnx_tensor);
    } else if (onnx_tensor.data_type() == TensorProto::UINT32) {
      ConvertIntegralValueToxcelerate<::google::protobuf::uint64>(
          c2_op, c2_values, onnx_tensor);
      // NOLINTNEXTLINE(bugprone-branch-clone)
    } else if (onnx_tensor.data_type() == TensorProto::BOOL) {
      ConvertIntegralValueToxcelerate<::google::protobuf::int8>(
          c2_op, c2_values, onnx_tensor);
    } else if (onnx_tensor.data_type() == TensorProto::UINT8) {
      ConvertIntegralValueToxcelerate<::google::protobuf::uint8>(
          c2_op, c2_values, onnx_tensor);
    } else if (onnx_tensor.data_type() == TensorProto::INT8) {
      ConvertIntegralValueToxcelerate<::google::protobuf::int8>(
          c2_op, c2_values, onnx_tensor);
    } else if (onnx_tensor.data_type() == TensorProto::UINT16) {
      ConvertIntegralValueToxcelerate<::google::protobuf::uint16>(
          c2_op, c2_values, onnx_tensor);
    } else if (onnx_tensor.data_type() == TensorProto::INT16) {
      ConvertIntegralValueToxcelerate<::google::protobuf::int16>(
          c2_op, c2_values, onnx_tensor);
    } else if (onnx_tensor.data_type() == TensorProto::INT32) {
      ConvertIntegralValueToxcelerate<::google::protobuf::int32>(
          c2_op, c2_values, onnx_tensor);
    ...
    if (onnx_tensor.data_type() == TensorProto::FLOAT) {
      c2_dtype->set_i(xcelerate::TensorProto::FLOAT);
      ...
      c2_dtype->set_i(xcelerate::TensorProto::DOUBLE);
      ...
      c2_dtype->set_i(xcelerate::TensorProto::INT64);
      ...
      c2_dtype->set_i(xcelerate::TensorProto::INT32);
...
bool xcelerateBackend::SupportOp(const std::string type) const {
