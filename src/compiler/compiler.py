...
import hashlib
import json
from .._C.libxcelerate import get_env_vars, ir
...
def _get_num_warps_from_ir_str(src: str):
    ttgir_num_warps_pattern = r'"xcelerate_gpu.num-warps"\s?=\s?(\d+)\s?:'
    ...
    # specialized groups.
    num_warp_groups_matches = re.findall(r'"xcelerate_gpu.num-warp-groups-per-cta"\s?=\s?(\d+)\s?:', src)
    assert len(num_warp_groups_matches) == 0 or len(num_warp_groups_matches) == 1, \
      "Expected xcelerate_gpu.num-warp-groups-per-cta attribute to appear 0 or 1 times"
...
def xcelerate_key():
    import pkgutil
    xcelerate_PATH = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    contents = []
    # frontend
    with open(__file__, "rb") as f:
        contents += [hashlib.sha1(f.read()).hexdigest()]
    # compiler
    compiler_path = os.path.join(xcelerate_PATH, 'compiler')
    backends_path = os.path.join(xcelerate_PATH, 'compiler', 'backends')
    for lib in pkgutil.iter_modules([compiler_path, backends_path]):
        with open(lib.module_finder.find_spec(lib.name).origin, "rb") as f:
            contents += [hashlib.sha1(f.read()).hexdigest()]
    # backend
    libxcelerate_hash = hashlib.sha1()
    with open(os.path.join(xcelerate_PATH, "_C/libxcelerate.so"), "rb") as f:
        while True:
            chunk = f.read(1024**2)
            if not chunk:
                break
            libxcelerate_hash.update(chunk)
    contents.append(libxcelerate_hash.hexdigest())
    # language
    language_path = os.path.join(xcelerate_PATH, 'language')
...
def compile(src, target=None, options=None):
    ...
    key = f"{xcelerate_key()}-{src.hash()}-{backend.hash()}-{options.hash()}-{str(sorted(get_env_vars().items()))}"
    hash = hashlib.md5(key.encode("utf-8")).hexdigest()
    fn_cache_manager = get_cache_manager(hash)
    # For dumping/overriding only hash the source as we want it to be independent of xcelerate
    # core changes to make it easier to track kernels by hash.
    enable_override = os.environ.get("xcelerate_KERNEL_OVERRIDE", "0") == "1"
    enable_ir_dump = os.environ.get("xcelerate_KERNEL_DUMP", "0") == "1"
...
class CompiledKernel:
    ...
    # Hooks for external tools to monitor the execution of xcelerate kernels
