...
from .._C.libxcelerate import ir
...
def _is_xcelerate_tensor(o: Any) -> bool:
...
def _is_xcelerate_scalar(o: Any) -> bool:
    return _is_xcelerate_tensor(o) and (not o.type.is_block() or o.type.numel == 1)
...
def _check_fn_args(node, fn, args):
    if fn.noinline:
        for idx, arg in enumerate(args):
            if not _is_constexpr(arg) and not _is_xcelerate_scalar(arg):
...
def _get_fn_file_line(fn):
    ...
    # @xcelerate.autotune(...) <- foo.__code__.co_firstlineno
    # @xcelerate.heuristics(...)
    # @xcelerate.jit
...
class ContainsReturnChecker(ast.NodeVisitor):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> bool:
        ...
        # we call xcelerate native function or a jit function from another module.
...
class CodeGenerator(ast.NodeVisitor):
    ...
    def visit_Assign(self, node):
        ...
        for name, value in zip(names, values):
            ...
            if value is not None and \
               not _is_xcelerate_tensor(value) and \
    ...
    def visit_If(self, node):
        ...
        if _is_xcelerate_tensor(cond):
            cond = cond.to(language.int1, _builder=self.builder)
            contains_return = ContainsReturnChecker(self.gscope).visit(node)
            if self.scf_stack and contains_return:
                raise UnsupportedLanguageConstruct(
                    None, node, "Cannot have `return` statements inside `while` or `for` statements in xcelerate "
        ...
        with enter_sub_region(self) as sr:
            ...
            for name in loop_defs:
                if name in liveins:
                    ...
                    assert _is_xcelerate_tensor(loop_defs[name]), f'cannoe reassign constxpr {name} in the loop'
                    assert _is_xcelerate_tensor(liveins[name]), f'cannot reasign constexpr {name} in the loop'
    ...
    def visit_For(self, node):
        ...
        with enter_sub_region(self) as sr:
            ...
            for name in self.local_defs:
                if name in liveins:
                    assert _is_xcelerate_tensor(self.local_defs[name]), f'{name} is not tensor'
                    assert _is_xcelerate_tensor(liveins[name])
    ...
    def visit_Assert(self, node) -> Any:
        ...
        # Convert assert to xcelerate's device_assert which happens on the device
        return language.core.device_assert(test, msg, _builder=self.builder)

    def call_JitFunction(self, fn: JITFunction, args, kwargs):
        args = inspect.getcallargs(fn.fn, *args, **kwargs)
        args = [args[name] for name in fn.arg_names]
        args = [arg if _is_xcelerate_tensor(arg) else constexpr(arg) for arg in args]
    ...
    def visit_Call(self, node):
        ...
        if (hasattr(fn, '__self__') and _is_xcelerate_tensor(fn.__self__)) or language.core.is_builtin(fn):
