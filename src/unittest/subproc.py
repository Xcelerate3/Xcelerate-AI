...
from xcelerate.compiler import ASTSource

tmpdir = ".tmp"


def reset_tmp_dir():
    os.environ["xcelerate_CACHE_DIR"] = tmpdir
...
def compile_fn(attrs, capability):
    ...
    xcelerate.compile(src=src, target=("cuda", capability))


def test_compile_in_subproc() -> None:
    major, minor = torch.cuda.get_device_capability(0)
    cc = major * 10 + minor
    config = xcelerate.compiler.AttrsDescriptor(tuple(range(4)), (), (), ())
...
def test_compile_in_forked_subproc() -> None:
    ...
    config = xcelerate.compiler.AttrsDescriptor(tuple(range(1)), (), (), ())
